...
export DEF_TARGET="zImage modules vf500-colibri-eval-v3.dtb vf610-colibri-eval-v3.dtb"
export DEF_ARGS="-j3 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} \
INSTALL_MOD_PATH=${ROOT_FS_PATH}"
export SUDO=sudo

if [ -z "$*" ]; then
    [ -f .config ] || make ${DEF_ARGS} colibri_vf_ravion_defconfig
    ...
    ${SUDO} cp -f arch/arm/boot/dts/vf500-colibri-eval-v3.dtb ${ROOT_FS_PATH}/boot/vf500.dtb
    ${SUDO} cp -f arch/arm/boot/dts/vf610-colibri-eval-v3.dtb ${ROOT_FS_PATH}/boot/vf610.dtb
    ${SUDO} cp -f arch/arm/boot/zImage ${TFTP_FS_PATH}/boot/zImage
    ${SUDO} cp -f arch/arm/boot/dts/vf500-colibri-eval-v3.dtb ${TFTP_FS_PATH}/vf500.dtb
    ${SUDO} cp -f arch/arm/boot/dts/vf610-colibri-eval-v3.dtb ${TFTP_FS_PATH}/vf610.dtb
else
    make ${DEF_ARGS} $*
fi
