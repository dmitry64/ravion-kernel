...
export DEF_TARGET="zImage modules imx6dl-colibri-cam-eval-v3.dtb imx6dl-colibri-eval-v3.dtb"
export DEF_ARGS="-j3 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} \
INSTALL_MOD_PATH=${ROOT_FS_PATH}"
export SUDO=sudo

if [ -z "$*" ]; then
    [ -f .config ] || make ${DEF_ARGS} colibri_imx6_defconfig
    ...
    ${SUDO} cp -f arch/arm/boot/dts/imx6dl-colibri-cam-eval-v3.dtb ${ROOT_FS_PATH}/boot/mx6-cam.dtb
    ${SUDO} cp -f arch/arm/boot/dts/imx6dl-colibri-eval-v3.dtb ${ROOT_FS_PATH}/boot/mx6.dtb
    ${SUDO} cp -f arch/arm/boot/zImage ${TFTP_FS_PATH}/boot/zImage
    ${SUDO} cp -f arch/arm/boot/dts/imx6dl-colibri-cam-eval-v3.dtb ${TFTP_FS_PATH}/boot/mx6-cam.dtb
    ${SUDO} cp -f arch/arm/boot/dts/imx6dl-colibri-eval-v3.dtb ${TFTP_FS_PATH}/boot/mx6.dtb
    if [ -d ${BR_OVERLAY_PATH} ]; then
	rm -rf \
	${BR_OVERLAY_PATH}/lib/modules/* \
	${BR_OVERLAY_PATH}/boot/zImage  \
	${BR_OVERLAY_PATH}/boot/mx6-cam.dtb \
	...
	cp ${ROOT_FS_PATH}/boot/mx6-cam.dtb	${BR_OVERLAY_PATH}/boot/
